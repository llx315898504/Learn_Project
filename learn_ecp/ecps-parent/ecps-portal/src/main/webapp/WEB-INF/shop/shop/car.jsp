...
function changeNum(skuId, quantity){
	if(quantity == 0){
		...
		if(result.ebSku.stockInventory < quantity){
			alert("当前商品库存不足"+quantity+"个仅有"+result.ebSku.stockInventory+"个");
			window.location.href = "${path}/cart/modifyCart.do?skuId="+skuId+"&modQuantity="+result.ebSku.stockInventory;
		}else{
		...
		<div class="ofc pb40">
			...
			<tbody>
				...
				<c:forEach items="${cartList }" var="cart">
					<tr>
					<td>${cart.ebSku.ebItem.itemNo }</td>
					<td class="nwp pic">
						<ul class="uls">
							<li>
								<a href="${path }/html/${cart.ebSku.ebItem.itemId}.html" title="" class="pic"><img src="${request_file_path }${cart.ebSku.ebItem.imgs}" alt="摩托罗拉ME525" /></a>
								<dl>
									<dt><a href="${path }/html/${cart.ebSku.ebItem.itemId}.html">${cart.ebSku.ebItem.itemName }</a></dt>
								</dl>
							</li>
					...
					<td>
						<c:forEach items="${cart.ebSku.ebSpecList }" var="spec">
							${spec.specValue }
						</c:forEach>
					</td>
					<td>￥${cart.ebSku.skuPrice }</td>
					<td>
					...
					<td>￥${cart.ebSku.skuPrice * cart.quantity }</td>
