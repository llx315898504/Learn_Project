...
$(function(){
	...
	$(".spec a").click(function(){
		...
		$.ajax({
			url:"${path}/item/selectSkuById.do",
			type:"post",
			dataType:"text",
			data:{
				skuId:skuId
			...
			success:function(responseText){
				...
				$("#skuPrice").html("￥"+jsonObj.ebSku.skuPrice);
				$("#marketPrice").html("￥"+jsonObj.ebSku.marketPrice);
				if(jsonObj.ebSku.stockInventory > 0){
					$("#stockState").html("有货");
					$("#buyNow").show();
					$("#addMyCart").show();
				}else{
							...
							<c:choose>
								<c:when test="${state.index == 0 }">
									<a href="javascript:void(0);"  class="here" skuId="${sku.skuId }" skuPrice="${sku.skuPrice }">
										<c:forEach items="${sku.ebSpecList }" var="spec">
											${spec.specValue }
										</c:forEach>
									</a>
								</c:when>
