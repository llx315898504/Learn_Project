...
<div class="frameR"><div class="content">
   ...
   <table cellspacing="0" summary="" class="tab" id="myTable">
    ...
    <tr>
    <td><a href="${path }/shop/order/orderPay/detail.jsp?orderId=3123">${tb.ebOrder.orderNum }</a></td>
    <td>${tb.ebOrder.shipName }</td>
    <td>${tb.ebOrder.phone }</td>
    <td>${tb.ebOrder.orderSum }</td>
    <td>在线支付 </td>
    <td>
    	<c:if test="${tb.ebOrder.isPaid == 1 }">已付</c:if>
    	<c:if test="${tb.ebOrder.isPaid == 0 }">未付</c:if>
    </td>
    <td class="nwp"><fmt:formatDate value="${tb.ebOrder.orderTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
    <td class="nwp"><fmt:formatDate value="${tb.ebOrder.updateTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
    <td class="nwp">
    	已付款单
    </td>
    <td><a href="${path }/order/viewDetail.do?orderId=${tb.ebOrder.orderId}&taskId=${tb.task.id}&dirName=orderCall">查看</a></td>
    </tr>
    </c:forEach>
    </table>
    </form>
