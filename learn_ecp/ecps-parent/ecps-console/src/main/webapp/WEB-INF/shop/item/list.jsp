...
<script type="text/javascript" src="${path}/ecps/console/res/js/item/list.js">
</script>
</head>
<body id="main">
...
<div class="frameR"><div class="content">
    ...
    <h2 class="h2_ch"><span id="tabs" class="l">
        ...
        <a id="label4" href="${path}/item/queryItemByCondtion.do?showStatus=1"   title="未上架实体商品" class="nor">未上架</a>
        <a id="label5" href="${path}/item/queryItemByCondtion.do?showStatus=0"  title="已上架实体商品" class="nor">已上架</a>
    </span></h2>

<form id="form1" name="form1" action="${path}/item/queryItemByCondtion.do" method="post">
    <input type="hidden" id="showStatus" value="${ReqVo.showStatus}" name="showStatus">
    <div class="sch">
        <p>搜索：
        <select id="brandId" name="brandId">
            <option value="">全部品牌</option>
            <c:forEach items="${bList}" var="brand">
            	<option value="${brand.brandId}" <c:if test="${ReqVo.brandId==brand.brandId}">selected</c:if>>${brand.brandName }</option>
            </c:forEach>
        </select>
        <select id="auditStatus" name="auditStatus" >
        	<option value="" selected>全部审核状态</option>
        	<option value="0" <c:if test="${ReqVo.auditStatus == 0}">selected</c:if> >待审核</option>
        	<option value="1" <c:if test="${ReqVo.auditStatus == 1}">selected</c:if>>通过</option>
        	<option value="2" <c:if test="${ReqVo.auditStatus == 2}">selected</c:if>>不通过</option>
        </select>
        <input type="text" id="searchText" value="${ReqVo.itemName}" name="itemName" title="请输入商品名称" class="text20 medium gray" />
	...
	<div class="page_c">
        ...
        <span class="r page">
            ...
            <a href="javascript:void(0);" id="previous" class="hidden" >上一页</a>
