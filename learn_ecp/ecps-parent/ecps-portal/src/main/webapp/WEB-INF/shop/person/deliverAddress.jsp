...
<script type="text/javascript" src="${path }/res/js/shop/person/deliverAddress.js"></script>
			...
			<div class="ofc">
				...
				<c:forEach items="${addrList }" var="addr">
					<tr <c:if test="${addr.defaultAddr == 1 }">class='here'</c:if> >
						...
						<td><a href="javascript:void(0);" title="修改" onclick="modify(${addr.shipAddrId})" class="blue">[修改]</a>
						<a href="${path}/user/login/deleteAddrById.do?shipAddrId=${addr.shipAddrId}" class="blue">[删除]</a></td>                  
					</tr>
				</c:forEach>
					...
					<li>
						...
						<select id="province" name="province">
							...
							<c:forEach items="${areaList }" var="area">
								<option value="${area.ereaId}">${area.ereaName}</option>
							</c:forEach>
						</select>
